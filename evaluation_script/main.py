...
def evaluate(test_annotation_file, user_submission_file, phase_codename, **kwargs):
    ...
    gold_labels, pred_labels, Q_evid_scores, QA_evid_scores, scores = [], [], [], [], []
    for test_sample, pred_sample in zip(gold_samples, pred_samples):
        gold_label, gold_Q_evidence, gold_QA_evidence = test_sample['label'], test_sample['Q_evidence'], test_sample['QA_evidence']
        pred_label, pred_Q_evidence, pred_QA_evidence = pred_sample['label'], pred_sample['Q_evidence'], pred_sample['QA_evidence']
        ...
        # Q evidence
        Q_score = compute_pairwise_evidence_score(gold_Q_evidence, pred_Q_evidence)
        Q_evid_scores.append(Q_score)

        # Q+A evidence
        QA_score = compute_pairwise_evidence_score(gold_QA_evidence, pred_QA_evidence)
        QA_evid_scores.append(QA_score)
    ...
    # evidence score
    Q_evidence_score = np.mean(Q_evid_scores)
    QA_evidence_score = np.mean(QA_evid_scores)
    # averitec score
        ...
        output["result"] = [
            {
                "dev_split": {
                    ...
                    "Q only": Q_evidence_score,
                    "Q + A": QA_evidence_score,
